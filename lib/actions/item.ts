...
export async function getItemsWithFavoriteAndCountByText(
    take:number,
    skip:number,
    text:string)
:Promise<{
    ...
    try {
        ...
        const [items, num] = await Promise.all([
            getItemsForMainPage(take,skip,text),
            getCountItemsByText(text)
        ]);

        if(!!items && typeof num !== "undefined" && Number.isInteger(num)){
            return {
                items:items,
                count:num,
            };
